...
// ─── Name pools ──────────────────────────────────────────────────────────────
const ENTRY_NAMES  = ['Ingress API', 'API Gateway', 'Web Frontend', 'Load Balancer'];
const MIDDLE_NAMES = [
  'Auth Service', 'User Service', 'Cache Layer', 'Session Store',
  'Search Service', 'Payment Service', 'Notification Service', 'Config Service',
  'Token Validator', 'Inventory Service', 'Order Service', 'Rate Limiter',
  'Recommendation Engine', 'Billing Service',
];
const EXIT_NAMES   = ['Primary DB', 'Query Cache', 'Message Queue', 'Blob Storage',
                      'Analytics Sink', 'Event Store'];
...
function buildTopology(rng) {
  ...
  for (let li = 0; li < template.length; li++) {
    ...
    for (let i = 0; i < size; i++) {
      ...
      layer.push({ id: `n${node_num++}`, name, max_concurrency, queue_limit, local_latency_ticks });
    }
    layers.push(layer);
  }
  ...
  for (let li = 0; li < layers.length - 1; li++) {
    for (const src of layers[li]) {
      for (const tgt of layers[li + 1]) {
        ...
        const sync_prob = li === 0 ? 0.40 : 0.65;
        ...
        edges.push({ source_id: src.id, target_id: tgt.id, mode, timeout_ticks });
      }
    }
  }
...
function effectiveSlotTime(node_id, nodes_map, edges) {
  ...
  const sync_kids = edges.filter(e => e.source_id === node_id && e.mode === 'SYNC');
  if (!sync_kids.length) return node.local_latency_ticks;
  const max_child = Math.max(...sync_kids.map(e => effectiveSlotTime(e.target_id, nodes_map, edges)));
...
function safeArrivalRate(nodes, edges, entry_id) {
  ...
  const min_tp = Math.min(...nodes.map(n => {
...
const STRESSOR_TYPES = ['LATENCY_SPIKE', 'CONCURRENCY_CRUSH', 'ARRIVAL_SPIKE', 'TIMEOUT_TRAP', 'QUOTA_IDENTITY_BUG'];
...
function applyStressor(rng, nodes, edges, arrival_rate, forced_type) {
  ...
  nodes = nodes.map(n => ({ ...n, token_bucket: n.token_bucket ? { ...n.token_bucket } : undefined }));
  edges = edges.map(e => ({ ...e }));

  // QUOTA_IDENTITY_BUG requires at least one bucketed node.
  const bucketed = nodes.filter(n => n.token_bucket);
  const available = STRESSOR_TYPES.filter(t => t !== 'QUOTA_IDENTITY_BUG' || bucketed.length > 0);
  ...
  if (type === 'LATENCY_SPIKE') {
    ...
    const sync_targets = new Set(edges.filter(e => e.mode === 'SYNC').map(e => e.target_id));
    const candidates   = nodes.filter(n => n.id !== nodes[0].id && sync_targets.has(n.id));
    const target = candidates.length ? pick(rng, candidates) : nodes[nodes.length - 1];
    ...
    description = `${target.name} is experiencing severe I/O degradation. ` +
      `Processing time: ${old_v} → ${new_v} ticks (${mult}×).`;
    mutation = { type, node_id: target.id, property: 'local_latency_ticks', old_value: old_v, new_value: new_v };
    ...
    description = `${target.name} is under severe memory pressure. ` +
      `Max concurrency: ${old_v} → ${new_v} (thread pool starved).`;
    mutation = { type, node_id: target.id, property: 'max_concurrency', old_value: old_v, new_value: new_v };
    ...
    mutation = { type, property: 'arrival_rate', old_value: old_v, new_value: new_v };
    ...
    edges.find(e => e.source_id === edge.source_id && e.target_id === edge.target_id).timeout_ticks = new_v;
    const src_name = nodes.find(n => n.id === edge.source_id).name;
    description = `Misconfigured timeout on ${src_name} → ${target_node.name}. ` +
      `timeout_ticks: ${old_v} → ${new_v} (well below downstream latency of ${target_node.local_latency_ticks}).`;
    mutation = { type, edge, property: 'timeout_ticks', old_value: old_v, new_value: new_v };

  } else { // QUOTA_IDENTITY_BUG
    ...
    nodes.find(n => n.id === target.id).token_bucket = { capacity: new_cap, refill_rate: new_ref };
    description = `${target.name} is forwarding its own service identity instead of the caller's. ` +
    ...
    mutation = { type, node_id: target.id, property: 'token_bucket',
      old_value: old_tb, new_value: { capacity: new_cap, refill_rate: new_ref } };
  }

  return { stressed_nodes: nodes, stressed_edges: edges, stressed_arrival_rate: arrival_rate, stressor: { type, description, mutation } };
}

// ─── Causal explanation builder ──────────────────────────────────────────────
function buildExplanation(failure, events, nodes_map, stressor) {
  if (!failure) return 'The system remained stable for the entire simulation window.';
  ...
  if (failure.type === 'RATE_LIMIT_DROP') {
    if (stressor.type === 'QUOTA_IDENTITY_BUG') {
      return `${failed_node} was forwarding all requests under its own service identity instead of propagating the caller's. ` +
    ...
    return `${failed_node}'s quota bucket was drained faster than it could refill under the traffic surge. ` +
  ...
  // QUEUE_DROP
  const prior_timeouts = events.filter(e => e.type === 'TIMEOUT_CASCADE' && e.tick < failure.tick);
  if (stressor.type === 'LATENCY_SPIKE' || stressor.type === 'CONCURRENCY_CRUSH') {
...
function generateScenario(seed) {
  ...
  (function verifyAndAdjust() {
    while (arrival_rate >= 1) {
      const check = new Simulator({
        nodes, edges, entry_node_id,
        arrival_rate, deadline_ticks: 0, max_retries: 0,
      });
  ...
  (function assignBuckets() {
    const lf = computeLoadFactors(nodes, edges, entry_node_id);
    for (const node of nodes.slice(1)) { // skip entry
      if (rng() < 0.35) {
  ...
  function runStressor(forced_type) {
    ...
    return { ...applied, result: r, quiz_event: qe, max_retries, deadline_ticks };
  }
  ...
  if (!best || !best.quiz_event) {
    for (const t of STRESSOR_TYPES) {
      ...
      if (attempt && attempt.quiz_event) { best = attempt; break; }
    }
  }

  const { stressed_nodes, stressed_edges, stressed_arrival_rate, stressor, result, quiz_event,
          max_retries, deadline_ticks } = best;
  ...
  return {
    ...
    answer: quiz_event
      ? { node_id: quiz_event.node_id, failure_type: quiz_event.type,
          tick: quiz_event.tick, explanation, node_peaks: result.node_peaks }
      : null,
  };
}
