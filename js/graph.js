...
class GraphRenderer {
  ...
  render(scenario) {
    ...
    const { stressed_nodes: nodes, stressed_edges: edges, layers, stressor } = scenario;
    ...
    for (const edge of edges) {
      ...
      const is_stressed = stressed_edge &&
  ...
  revealAnswer(answer_node_id, user_node_id) {
    ...
    if (user_node_id && user_node_id !== answer_node_id && this._node_els[user_node_id]) {
  ...
  _drawNode(svg, node, pos, is_stressed) {
    const g = svgEl('g', { class: 'node' + (is_stressed ? ' node--stressed' : ''), 'data-id': node.id });
    ...
    // Node name.
    g.appendChild(svgText(node.name, {
      x: pos.cx, y: pos.y + 20,
      'text-anchor': 'middle',
      class: 'node-name',
    }));
    ...
    // Capacity stats.
    g.appendChild(svgText(`conc: ${node.max_concurrency}  q: ${node.queue_limit}`, {
      x: pos.cx, y: pos.y + 38,
      'text-anchor': 'middle',
      class: 'node-stat',
    }));

    g.appendChild(svgText(`latency: ${node.local_latency_ticks} ticks`, {
      x: pos.cx, y: pos.y + 54,
      'text-anchor': 'middle',
      class: 'node-stat',
    }));
    ...
    if (node.token_bucket) {
      g.appendChild(svgText(`quota: ${node.token_bucket.capacity}  +${node.token_bucket.refill_rate}/t`, {
        x: pos.cx, y: pos.y + 70,
        'text-anchor': 'middle',
        class: 'node-quota',
      }));
    ...
    // Stress indicator.
    if (is_stressed) {
      g.appendChild(svgText('⚡ STRESSOR', {
        x: pos.cx, y: pos.y + 84,
        'text-anchor': 'middle',
  ...
  _drawEdge(svg, from, to, edge, is_stressed) {
    ...
    const color_class = is_stressed ? 'stress' : (edge.mode === 'SYNC' ? 'sync' : 'async');
    ...
    if (edge.mode === 'SYNC') {
      ...
      lbl_g.appendChild(svgEl('rect', {
        x: lx - label_w / 2, y: ly - 10,
        width: label_w, height: 20,
        rx: 2,
        class: 'edge-label-bg',
      }));

      const txt = svgEl('text', { 'text-anchor': 'middle', class: `edge-label edge-label--${color_class}` });
      ...
      lbl_g.appendChild(svgEl('rect', {
        x: lx - label_w / 2, y: ly - 9,
        width: label_w, height: 12,
        rx: 2,
        class: 'edge-label-bg',
      }));
      lbl_g.appendChild(svgText('ASYNC', {
        x: lx, y: ly,
        'text-anchor': 'middle',
