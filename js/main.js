...
function loadScenario(seed) {
  ...
  const retry_str = scenario.max_retries > 0
...
function submitGuess() {
  ...
  // Verdict text.
  const correct_name = scenario.nodes.find(n => n.id === answer.node_id)?.name ?? answer.node_id;
  if (fully_correct) {
    $verdict.innerHTML = `<i class="ph-light ph-check-circle"></i> Correct. Your system analysis hat is working.`;
    $verdict.className = 'verdict--correct';
  } else if (correct_node) {
    $verdict.innerHTML = `<i class="ph-light ph-warning"></i> Right node, wrong failure type. It was: ${labelForType(answer.failure_type)}.`;
    $verdict.className = 'verdict--partial';
  } else if (correct_type) {
    $verdict.innerHTML = `<i class="ph-light ph-warning"></i> Right failure type, wrong node. The failure was at: ${correct_name}.`;
    $verdict.className = 'verdict--partial';
  } else {
    $verdict.innerHTML = `<i class="ph-light ph-x-circle"></i> Incorrect. Failed at ${correct_name} (${labelForType(answer.failure_type)}) at tick ${answer.tick}.`;
...
function labelForType(type) {
  return { QUEUE_DROP: 'Queue Drop (OOM)', TIMEOUT_CASCADE: 'Timeout Cascade', DEADLINE_EXCEEDED: 'Deadline Exceeded', RATE_LIMIT_DROP: 'Rate Limited' }[type] ?? type;
}

// ─── Event listeners ─────────────────────────────────────────────────────────
$type_btns.forEach(b => b.addEventListener('click', () => onTypeClick(b.dataset.type)));
