...
describe('Generator', function () {
  ...
  describe('Baseline stability', function () {
    ...
    it('the unstressed system is stable (no failure within 400 ticks)', function () {
      for (const seed of SEEDS) {
        ...
        expect(result.failure, `seed ${seed}: baseline should be stable`).to.not.exist;
      }
    });
  ...
  describe('Topology integrity', function () {
    ...
    it('every scenario has at least one entry node and one exit node', function () {
      for (const seed of SEEDS) {
        ...
        const entry_candidates = [...node_ids].filter(id => !has_incoming.has(id));
        const exit_candidates  = [...node_ids].filter(id => !has_outgoing.has(id));

        expect(entry_candidates.length, `seed ${seed}: must have ≥1 entry`).to.be.greaterThan(0);
        expect(exit_candidates.length,  `seed ${seed}: must have ≥1 exit`).to.be.greaterThan(0);
        expect(s.entry_node_id, `seed ${seed}: entry_node_id must be declared`).to.exist;
    ...
    it('all edge endpoints reference existing nodes', function () {
      for (const seed of SEEDS) {
        ...
        for (const e of s.edges) {
          expect(node_ids.has(e.source_id), `seed ${seed}: source ${e.source_id} must exist`).to.be.true;
          expect(node_ids.has(e.target_id), `seed ${seed}: target ${e.target_id} must exist`).to.be.true;
        }
      }
    });
    ...
    it('the graph is acyclic (no node appears on its own reachability path)', function () {
      for (const seed of SEEDS) {
        ...
        for (const n of s.nodes) {
          expect(has_cycle(n.id), `seed ${seed}: cycle detected from ${n.id}`).to.be.false;
        }
      }
    });
    ...
    it('all nodes have valid positive capacities', function () {
      for (const seed of SEEDS) {
        ...
        for (const n of [...s.nodes, ...s.stressed_nodes]) {
          expect(n.max_concurrency,    `${n.name} max_concurrency`).to.be.greaterThan(0);
          expect(n.queue_limit,        `${n.name} queue_limit`).to.be.greaterThan(0);
          expect(n.local_latency_ticks,`${n.name} local_latency_ticks`).to.be.greaterThan(0);
        }
      }
    });
  ...
  describe('Stressor properties', function () {
    ...
    it('every stressor has a non-empty description', function () {
      for (const seed of SEEDS) {
        const s = generateScenario(seed);
        expect(s.stressor.description, `seed ${seed}`).to.be.a('string').with.length.greaterThan(10);
      }
    });

    it('LATENCY_SPIKE stressor increases latency (stressed > baseline)', function () {
      const spike_seeds = SEEDS.filter(seed => generateScenario(seed).stressor.type === 'LATENCY_SPIKE');
      for (const seed of spike_seeds) {
        ...
        expect(stressed.local_latency_ticks).to.be.greaterThan(baseline.local_latency_ticks);
      }
    });

    it('CONCURRENCY_CRUSH stressor reduces concurrency (stressed < baseline)', function () {
      const crush_seeds = SEEDS.filter(seed => generateScenario(seed).stressor.type === 'CONCURRENCY_CRUSH');
      for (const seed of crush_seeds) {
        const s = generateScenario(seed);
        const m = s.stressor.mutation;
        const baseline = s.nodes.find(n => n.id === m.node_id);
        const stressed = s.stressed_nodes.find(n => n.id === m.node_id);
        expect(stressed.max_concurrency).to.be.lessThan(baseline.max_concurrency);
      }
    });

    it('ARRIVAL_SPIKE stressor increases arrival rate (stressed > baseline)', function () {
      const spike_seeds = SEEDS.filter(seed => generateScenario(seed).stressor.type === 'ARRIVAL_SPIKE');
      for (const seed of spike_seeds) {
        const s = generateScenario(seed);
    ...
    it('TIMEOUT_TRAP sets timeout below downstream latency', function () {
      const trap_seeds = SEEDS.filter(seed => generateScenario(seed).stressor.type === 'TIMEOUT_TRAP');
      for (const seed of trap_seeds) {
        ...
        const stressed_edge = s.stressed_edges.find(
          e => e.source_id === m.edge.source_id && e.target_id === m.edge.target_id
        );
        expect(stressed_edge.timeout_ticks).to.be.lessThan(target.local_latency_ticks);
      }
    });
  ...
  describe('Answer validity', function () {
    ...
    it('answer node_id always references an existing node', function () {
      for (const seed of SEEDS) {
        ...
        expect(ids).to.include(s.answer.node_id, `seed ${seed}: answer node must exist`);
      }
    });

    it('answer failure_type is one of the known types', function () {
      const valid_types = ['QUEUE_DROP', 'TIMEOUT_CASCADE', 'DEADLINE_EXCEEDED', 'RATE_LIMIT_DROP'];
      for (const seed of SEEDS) {
        const s = generateScenario(seed);
        if (!s.answer) continue;
